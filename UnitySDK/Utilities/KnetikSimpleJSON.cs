...
namespace KnetikSimpleJSON
{
    ...
    public class KnetikJSONNode
    {
        ...
        public virtual List<KnetikJSONNode> Children { get { return null; } }
        ...
        protected static StringBuilder builder = null;
        private const int BuilderMaxCapacity = 4096;
        
        protected static void AllocateBuilder(int capacity)
        {
            if (builder == null) {
                builder = new StringBuilder(Math.Min(capacity, BuilderMaxCapacity));
            }
            builder.Length = 0;
        }
        
        protected static void FreeBuilder()
        {
            if (builder.Capacity > BuilderMaxCapacity) {
                builder = null;
            }
        }
                ...
                switch (aJSON[i])
                {
                    ...
                    case ']':
                        ...
                        if (builder.Length != 0)
                        {
                            TokenName = TokenName.Trim();
                            if (ctx is KnetikJSONArray)
                                ctx.Add(builder.ToString());
                            else if (TokenName != "")
                                ctx.Add(TokenName,builder.ToString());
                        }
                        TokenName = "";
                        builder.Length = 0;
                        if (stack.Count>0)
                        ...
                        if (QuoteMode)
                        {
                            ...
                            switch (C)
                            {
                                ...
                                case 'u':
                                {
                                    ...
                                    builder.Append((char)int.Parse(s, System.Globalization.NumberStyles.AllowHexSpecifier));
                                    i += 4;
                                    break;
                                }
    ...
    public class KnetikJSONArray : KnetikJSONNode, IEnumerable
    {
        ...
        public override List<KnetikJSONNode> Children
        {
            get
            {
                return m_List;
        ...
        public List<KnetikJSONNode>.Enumerator GetEnumerator()
        {
    ...
    public class KnetikJSONClass : KnetikJSONNode, IEnumerable
    {
        ...
        private List<KnetikJSONNode> iterList = null;
        public override KnetikJSONNode this[string aKey]
        {
            get
            {
        ...
        public override List<KnetikJSONNode> Children
        {
            ...
            {
                if (iterList == null) {
                    iterList = new List<KnetikJSONNode>(m_Dict.Count);
                    var enumerator = m_Dict.Values.GetEnumerator();
                    while (enumerator.MoveNext()) {
                        iterList.Add(enumerator.Current);
                    }
                }
                return iterList;
            }
        ...
        public Dictionary<string, KnetikJSONNode>.Enumerator GetEnumerator()
        {
